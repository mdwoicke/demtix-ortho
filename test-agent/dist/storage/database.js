...
class Database {
    ...
    saveGeneratedFix(fix) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(fix.fixId, fix.runId, fix.type, fix.targetFile, fix.changeDescription, fix.changeCode, fix.location ? JSON.stringify(fix.location) : null, fix.priority, fix.confidence, JSON.stringify(fix.affectedTests), fix.rootCause ? JSON.stringify(fix.rootCause) : null, fix.status, fix.createdAt || new Date().toISOString());
    }
    /**
