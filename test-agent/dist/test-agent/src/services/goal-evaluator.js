...
class GoalEvaluator {
     ...
     */
    evaluateTest(testCase, progress, conversationHistory, durationMs) {
        // Evaluate each goal
        const goalResults = this.evaluateAllGoals(testCase.goals, progress, conversationHistory);
        // Check constraints
        const constraintViolations = this.checkAllConstraints(testCase.constraints, progress, conversationHistory, durationMs);
        // Determine overall pass/fail
        const passed = this.determinePassFail(testCase.goals, goalResults, constraintViolations);
        // Generate summary
        const summary = this.generateSummary(passed, goalResults, constraintViolations, progress);
        return {
            passed,
            goalResults,
     ...
     * Evaluate all goals
     */
    evaluateAllGoals(goals, progress, conversationHistory) {
     ...
     */
    evaluateGoal(goal, context, progress) {
     ...
     */
    evaluateDataCollectionGoal(goal, progress) {
     ...
     */
    evaluateBookingGoal(goalId, context, progress) {
     ...
     */
    evaluateTransferGoal(goalId, context, progress) {
     ...
     */
    evaluateConversationEndedGoal(goalId, progress) {
     ...
     */
    evaluateErrorHandledGoal(goalId, context, progress) {
     ...
     */
    checkAllConstraints(constraints, progress, conversationHistory, durationMs) {
        const violations = [];
        const context = this.buildGoalContext(progress, conversationHistory);
     ...
     */
    toTranscriptTurn(backendTurn) {
        return 2 * backendTurn;
    }
    /**
     * Check a single constraint
     */
    checkConstraint(constraint, context, progress, durationMs) {
     ...
     * Determine overall pass/fail
     */
    determinePassFail(goals, goalResults, violations) {
     ...
     */
    generateSummary(passed, goalResults, violations, progress) {
        const parts = [];
        // Overall status
     ...
     */
    buildGoalContext(progress, conversationHistory) {
        return {
            collectedData: progress.collectedFields,
            conversationHistory,
            // Include persistent flags that survive subsequent intents
            agentConfirmedBooking: progress.bookingConfirmed || // Persistent flag
                progress.lastAgentIntent === 'confirming_booking' ||
                progress.currentFlowState === 'confirmation',
            agentInitiatedTransfer: progress.transferInitiated || // Persistent flag
