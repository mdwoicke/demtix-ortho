...
class ExperimentService {
    constructor(db, variantService) {
        this.db = db;
        this.variantService = variantService;
        this.statisticsService = new statistics_service_1.StatisticsService(db);
    }
    /**
    ...
    createExperiment(input) {
        ...
        // Build traffic split
        const trafficSplit = input.trafficSplit || {};
     ...
     */
    completeExperiment(experimentId, conclusion) {
        const experiment = this.db.getExperiment(experimentId);
        if (!experiment) {
            throw new Error(`Experiment ${experimentId} not found`);
        }
     ...
     */
    selectVariant(experimentId, testId) {
        const experiment = this.db.getExperiment(experimentId);
        if (!experiment) {
            throw new Error(`Experiment ${experimentId} not found`);
        }
     ...
     */
    recordExperimentRun(run) {
        return this.db.saveExperimentRun(run);
    }
    /**
     * Record a test result as an experiment run
     */
    recordTestResult(experimentId, runId, testId, variantSelection, result) {
        const now = new Date().toISOString();
        const metrics = {
    ...
    getExperimentSummary(experimentId) {
        ...
        const analysis = experiment.status === 'running' || experiment.status === 'completed'
            ...
            : null;
        const controlRuns = runCounts.find(r => experiment.variants.find(v => v.variantId === r.variantId && v.role === 'control'));
        const treatmentRuns = runCounts.find(r => experiment.variants.find(v => v.variantId === r.variantId && v.role === 'treatment'));
        return {
