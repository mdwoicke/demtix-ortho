...
const agent_failure_analyzer_1 = require("./analysis/agent-failure-analyzer");
 ...
 */
function loadGoalTestsFromDatabase() {
    const dbPath = path_1.default.resolve(__dirname, '../data/test-results.db');
 ...
 */
async function runGoalTests(scenarioIds, concurrency, db) {
    ...
    console.log(`[GoalTest] Loaded ${goal_happy_path_1.goalHappyPathScenarios.length} TS scenarios + ${dbScenarios.length} DB scenarios`);
...
program
    ...
    try {
        ...
        const analyzer = new agent_failure_analyzer_1.AgentFailureAnalyzer(db);
        ...
        for (let i = 0; i < fixes.length; i++) {
            ...
            if (fix.location) {
                if (fix.location.section)
                    console.log(` Section: ${fix.location.section}`);
                if (fix.location.function)
                    console.log(` Function: ${fix.location.function}`);
            }
...
program
    ...
    try {
        ...
        db.updateFixStatus(fixId, status);
