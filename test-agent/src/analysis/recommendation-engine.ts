...
import { Finding } from '../tests/test-case';
...
export interface Recommendation {
  ...
  // For Flowise prompts - copy-paste ready
  promptSuggestion?: {
    location: string;
...
export class RecommendationEngine {
   ...
   */
  private groupFindings(results: TestResult[]): FindingGroup[] {
   ...
   */
  private getFindingGroupKey(finding: Finding): string {
   ...
   */
  private createRecommendation(group: FindingGroup): Recommendation | null {
   ...
   */
  private determineRecommendationType(finding: Finding): Recommendation['type'] {
    switch (finding.type) {
      case 'prompt-issue':
   ...
   */
  private calculatePriority(group: FindingGroup): number {
    ...
    if (group.affectedTests.length > 3) priority += 2;
    else if (group.affectedTests.length > 1) priority += 1;
   ...
   */
  private generateTitle(group: FindingGroup): string {
   ...
   */
  private generateSolution(group: FindingGroup, recType: Recommendation['type']): {
    text: string;
    promptSuggestion?: Recommendation['promptSuggestion'];
    toolSuggestion?: Recommendation['toolSuggestion'];
  } {
    const finding = group.findings[0];
   ...
   */
  private generatePromptSuggestion(finding: Finding): Recommendation['promptSuggestion'] {
    // Analyze the finding to generate appropriate prompt update
    let location = 'System Prompt';
    let changeType: 'append' | 'replace' | 'prepend' = 'append';
    let suggestedText = '';
...
- Offer to help with something else`;
   ...
   */
  private generateToolSuggestion(finding: Finding): Recommendation['toolSuggestion'] {
