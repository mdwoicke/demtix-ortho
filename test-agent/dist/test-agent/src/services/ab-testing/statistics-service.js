...
class StatisticsService {
     ...
     */
    calculateVariantStats(experimentId, variantId) {
     ...
     */
    performChiSquareTest(control, treatment, significanceThreshold = 0.05) {
        // Handle edge cases
        ...
        const chiSquare = Math.pow(Math.abs(controlPassed - expectedControlPassed) - 0.5, 2) / expectedControlPassed +
     ...
     */
    performTTest(controlValues, treatmentValues, significanceThreshold = 0.05) {
        // Handle edge cases
     ...
     */
    calculateProportionCI(successes, total, confidence = 0.95) {
        if (total === 0) {
    ...
    shouldConcludeExperiment(experimentId) {
        ...
        if (analysis.controlSampleSize >= experiment.maxSampleSize ||
